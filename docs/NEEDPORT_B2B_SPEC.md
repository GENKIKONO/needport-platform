...
### 提案作成UI + コンテンツ審査Linter + 管理側承認の連携（Phase 1.12）
提案作成フォーム、コンテンツ審査Linter、管理側承認ワークフローを実装しました。

#### 範囲
- **提案作成UI**: 構造化された提案ドラフト作成フォーム（ベンダー名、価格、期間、成果物、リスク、条件）
- **コンテンツLinter**: 連絡先直書き・外部リンク誘導の検出（エラー/警告）
- **承認ワークフロー**: Admin Consoleでの承認/却下/注目操作
- **比較UI統合**: 承認済み提案をデモ提案とマージ表示（featured先頭）

#### 本番化プラン
本番化時は以下の置換を行います：
- `/api/needs/[id]/proposals` POST → pendingドラフト作成
- Admin承認操作 → 提案ステータス更新API
- 比較UI → 承認済み提案の永続化表示
- Hireボタン → 契約開始ワークフロー接続

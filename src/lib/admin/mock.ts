...
const MOCK_NEEDS: NeedDetail[] = Array.from({length:50}).map((_,i)=> {
...
export function listNeeds(params:{ q?:string; stage?:Stage; page?:number; pageSize?:number; sort?:string }) {
  const { q, stage, page=1, pageSize=20 } = params ?? {};
  let rows = [...MOCK_NEEDS];
  if (q) rows = rows.filter(n => n.id.includes(q) || n.title.includes(q));
  if (stage) rows = rows.filter(n => n.stage === stage);
  const total = rows.length;
  const start = (page-1)*pageSize;
  return { total, items: rows.slice(start, start+pageSize) as NeedRow[] };
}

export function getNeed(id:string) {
  return MOCK_NEEDS.find(n => n.id === id) || null;
}

...
const schema = z.object({
  ...
  finalPrice: z.number().int().positive().max(10_000_000),
  feeRate: z.number().min(0.05).max(0.30).default(0.09)
});
...
export async function POST(req: Request) {
  ...
  const ref = `NP-${Date.now()}-${Math.floor(Math.random()*1000)}`;
  ...
  await sadmin.from('audit_logs').insert({
    ...
    target_id: data.id,
    meta: { method: 'bank_transfer', final_price: finalPrice, fee_rate: feeRate }
  });

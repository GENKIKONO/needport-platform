...
for (const filePath of routeFiles) {
  ...
  if (needsFixing) {
    ...
    if (!${varName}) {
      ...
      // Replace supabaseAdmin() calls
      content = content.replace(/supabaseAdmin\(\)/g, 'createAdminClientOrNull()!');
      // Replace helper function calls
      content = content.replace(/const\s+(\w+)\s*=\s*supabaseAdmin\(\)/g, (match, varName) => {
        return `const ${varName} = createAdminClientOrNull();
